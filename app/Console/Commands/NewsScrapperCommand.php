...
class NewsScrapperCommand extends BaseScrapper
{
    ...
    {
        $response = $this->cli->request('GET', 'https://arzdigital.com/');

        $news_arr = [];

        $response->filter('.arz-breaking-news__list.arz-row-sb')->children()->each(function ($node) use (&$news_arr) {
